...
public class UserManager extends EntityManager {
	...
	public ArrayList<ArrayList<String>> getUserListWithTotalInvoiceAmount() {
		...
		try {
	        ...
	        ArrayList<ArrayList<String>> tableContent = databaseOperations.getTableContent("user u", userAttributes, null, null, "u.id", null);
	        for (int currentIndex = 0; currentIndex < tableContent.size(); currentIndex++) {
	            ...
	            ArrayList<String> userTotalInvoiceValue = databaseOperations.executeStoredRoutine("calculate_user_total_invoice_value", parameterTypes, inputParameterValues, outputParameterDataTypes);
