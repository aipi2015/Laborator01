...
public class EntityManager {
	...
	public ArrayList<String> read(long id) {
		...
		try {
			...
			ArrayList<ArrayList<String>> result = databaseOperations.getTableContent(table, null, databaseOperations.getTablePrimaryKey(table) + "=" + id, null, null, null);
			if (result.size() != 1) {
				if (Constants.DEBUG) {
	...
	public int update(ArrayList<String> values, long id) {
		...
		try {
			databaseOperations = DatabaseOperationsImplementation.getInstance();
			int result = databaseOperations.updateRecordsIntoTable(table, null, values, databaseOperations.getTablePrimaryKey(table) + "=" + id);
			if (result != 1) {
				if (Constants.DEBUG) {
	...
	public int delete(long id) {
		...
		try {
			databaseOperations = DatabaseOperationsImplementation.getInstance();
			int result = databaseOperations.deleteRecordsFromTable(table, null, null, databaseOperations.getTablePrimaryKey(table) + "=" + id);
			if (result != 1) {
				if (Constants.DEBUG) {
	...
	public ArrayList<ArrayList<String>> getCollection(ArrayList<String> attributes, String tableAlias) {
		...
		try {
			databaseOperations = DatabaseOperationsImplementation.getInstance();
			return databaseOperations.getTableContent(table + (tableAlias != null ? " " + tableAlias : ""), attributes, null, null, null, null);
		} catch (SQLException sqlException) {
			System.out.println("An exception has occurred: " + sqlException.getMessage());
			if (Constants.DEBUG) {
				sqlException.printStackTrace();
	...
	public CachedRowSet getCollection(String matchColumn, ArrayList<String> attributes, String tableAlias) {
		try {
	        ...
	        result.setCommand("SELECT "+ attributesList.toString() + " FROM " + table + (tableAlias != null ? " " + tableAlias : ""));
